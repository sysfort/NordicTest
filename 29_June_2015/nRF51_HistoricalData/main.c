...
#define DEVICE_NAME                    "Nordic_nRF51"                            /**< Name of device. Will be included in the advertising data. */
...
#define MAX_BATTERY_LEVEL              140                                         /**< Maximum battery level as returned by the simulated measurement function. */
...
static ble_gls_t                       m_gls;                                      /**< Structure used to identify the History service. */
...
/**@brief Function for updating History measurement and updating History characteristic in History.
          Service.
 */
static void read_History_measurement(void)
{
    ble_gls_rec_t rec;
    uint32_t      err_code;

    static int16_t s_mantissa = 140;
    static int16_t s_exponent = -3;
    static uint8_t s_secs     = 5;

    // simulate the reading of a History measurement.
    rec.meas.flags                          = BLE_GLS_MEAS_FLAG_TIME_OFFSET   |
                                              BLE_GLS_MEAS_FLAG_CONC_TYPE_LOC |
                                              BLE_GLS_MEAS_FLAG_UNITS_MOL_L;
    rec.meas.base_time.year                 = 2015;
    rec.meas.base_time.month                = 6;
    rec.meas.base_time.day                  = 26;
 ...
 *
 * @details Initialize the History, Battery and Device Information services.
 */
static void services_init(void)
{
    ...
    // Initialize History Service - sample selection of feature bits.
    ...
    bas_init.initial_batt_level   = 140;
