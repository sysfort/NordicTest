...
import com.bluethhoth.testnordic.R;
...
*/                
	...
	private void showAboutDialog() {
		mAboutDialog = new Dialog(BluethoothActivity.this);
...
class TerminalEmulator {
    ...
    public void append(byte[] buffer, int base, int length) {
        for (int i = 0; i < length; i++) {
            ...
            try {
                if (BluethoothActivity.LOG_CHARACTERS_FLAG) {
                    char printableB = (char) b;
                    if (b < 32 || b > 126) {
                        printableB = ' ';
                    }
                    Log.w(BluethoothActivity.LOG_TAG, "'" + Character.toString(printableB) + "' (" + Integer.toString(b) + ")");
                }
                ...
                Log.e(BluethoothActivity.LOG_TAG, "Exception while processing character "
    ...
    private void selectGraphicRendition() {
        for (int i = 0; i <= mArgIndex; i++) {
            ...
            if (code == 0) { // reset
                ...
                if (BluethoothActivity.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
                    Log.w(BluethoothActivity.LOG_TAG, String.format("SGR unknown code %d", code));
                }
            }
        }
    }
    ...
    private void logError(String error) {
        if (BluethoothActivity.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
            Log.e(BluethoothActivity.LOG_TAG, error);
        }
        finishSequence();
    }
...
class EmulatorView extends View implements GestureDetector.OnGestureListener {
    ...
    private BluethoothActivity mBlueTerm;
...
*/
     ...
     */
    public void initialize(BluethoothActivity blueTerm) {
    	mBlueTerm = blueTerm;
        mTextSize = 15;
        mForeground = BluethoothActivity.WHITE;
        mBackground = BluethoothActivity.BLACK;
        updateText();
